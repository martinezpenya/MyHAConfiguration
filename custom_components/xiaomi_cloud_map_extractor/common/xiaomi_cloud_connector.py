...
class XiaomiCloudConnector:
    ...
    def login_step_2(self) -> bool:
        ...
        # Xiaomi endpoint behavior can vary; keep legacy params call and fallback to form-data.
        response = None
        try:
        ...
        if response is None or response.status_code != 200:
            try:
                response = self._session.post(url, headers=headers, data=fields, timeout=10)
            except:
                response = None
        ...
        if successful:
            ...
            if successful:
                ...
                if "notificationUrl" in json_resp:
                    ...
                    successful = None
                elif "desc" in json_resp:
                    _LOGGER.error("Xiaomi cloud auth failed: %s", json_resp["desc"])
        elif response is not None:
            _LOGGER.error("Xiaomi cloud auth failed with HTTP status %s", response.status_code)
        else:
            _LOGGER.error("Xiaomi cloud auth request failed (no response)")
